                            ...
                            <div class="card text-center mb-3 tim-item" data-status-tim="{{ $item->tim->status_tim }}">
                                <div class="card-body">
                                    ...
                                    @elseif ($item->status_presentasi == 'sedang_presentasi')
                                        <span class="badge bg-label-success text-capitalize">Sedang Presentasi</span>
                                    @endif
                                    <h5 class="card-title my-2 text-capitalize" data-bs-placement="bottom"
                                        data-bs-toggle="tooltip" title="{{ $item->judul }}">
                                        {{ Str::limit($item->judul, 20) }}</h5>
                                    <p class="card-text" data-bs-toggle="tooltip" data-popup="tooltip-custom"
                                        data-bs-placement="bottom" title="Jadwal Presentasi">
                                        {{ $item->jadwal_ke }} {{ \Carbon\Carbon::parse($item->mulai)->format('H:i') }} -
                                        {{ \Carbon\Carbon::parse($item->akhir)->format('H:i') }}
...
@section('script')
    <script>
        ...
        $('#selesaiPresentasiForm').on('submit', function(event) {
            event.preventDefault();

            var statusRevisi = $('input[name="status_revisi"]:checked').val();
            var statusPresentasi = $('input[name="status_presentasi"]:checked').val();

            if (!statusRevisi) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Peringatan',
                    text: 'Status Revisi harus di isi!',
                    showConfirmButton: false,
                    timer: 2000
                });
            } else if (!statusPresentasi) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Peringatan',
                    text: 'Status Presentasi harus di isi!',
                    showConfirmButton: false,
                    timer: 2000
                });
            } else {
                // Jika sudah diisi, lanjutkan dengan submit form
                this.submit();
            }
        });
